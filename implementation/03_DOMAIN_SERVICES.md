...
AI-02 wraps those executors in a bounded Responses API tool loop. Store and network conversations use separate strict schemas and prompts; network scope is rechecked against the exact authorized context set before provider invocation. Provider response storage is disabled. Reasoning effort and an output budget that includes reasoning tokens are explicit configuration, and an incomplete provider response cannot be misclassified as invalid user input. The UI renders evidence traces and limitations outside the model prose, so grounding remains inspectable even when answer wording varies.

AI-03 adds one store-only write tool whose sole effect is creating an evidence-backed `draft` with `executionStatus: not_executed`. A bounded explicit draft intent forces one deterministic read before forcing the draft tool; arbitrary analysis requests retain automatic tool choice. The service grounds the document from read-tool results collected in the same turn, persists model and prompt versions, and audits creation. Approval/rejection is a separate idempotent service requiring `recommendations.approve`; it records a decision snapshot and audit without executing operational changes.

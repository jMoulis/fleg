...
export async function ensureFoundationIndexesForDb(db: Db): Promise<void> {
  ...
  await Promise.all([
    ...
    db.collection("documentProcessingJobs").createIndex(
      { organizationId: 1, storeId: 1, state: 1, nextAttemptAt: 1 },
      { name: "document_processing_due" },
    ),
    db.collection("documentProcessingJobs").createIndex(
      { expiresAt: 1 }, { expireAfterSeconds: 0, name: "document_processing_expiry" },
    ),
    db.collection("documentProcessingLanes").createIndex(
      { organizationId: 1, storeId: 1 }, { unique: true, name: "document_processing_lane_scope" },
    ),
    ...
    db.collection("promotionContextObservations").createIndex(
      { organizationId: 1, storeId: 1, businessDate: 1, recordedAt: 1 },
      { name: "promotion_context_scope_date_recorded" },
    ),
    db.collection("promotionContextObservations").createIndex(
      { organizationId: 1, storeId: 1, productIds: 1, businessDate: 1 },
      { name: "promotion_context_scope_product_date" },
    ),
    db.collection("weatherContextObservations").createIndex(
      { organizationId: 1, storeId: 1, businessDate: 1, recordedAt: 1 },
      { name: "weather_context_scope_date_recorded" },
    ),
    db.collection("contextObservationCommands").createIndex(
      { organizationId: 1, storeId: 1, idempotencyKey: 1 },
      {
        unique: true,
        name: "context_observation_commands_scope_key_unique",
      },
    ),
    ...
    db.collection("orderSuggestionDrafts").createIndex(
      { organizationId: 1, storeId: 1, orderDate: 1, generatedAt: -1 },
      { name: "order_suggestions_scope_date_generated" },
    ),
    db.collection("orderSuggestionDrafts").createIndex(
      { organizationId: 1, storeId: 1, status: 1, deliveryDate: 1 },
      { name: "order_suggestions_scope_status_delivery" },
    ),
    db.collection("orderSuggestionCommands").createIndex(
      { organizationId: 1, storeId: 1, idempotencyKey: 1 },
      { unique: true, name: "order_suggestion_commands_scope_key_unique" },
    ),

...
import { getWeeklySalesView } from "@/server/services/granular-sales-service";
...
export async function GET(request: Request, routeContext: RouteContext) {
  ...
  try {
    ...
    const result = await getWeeklySalesView(context, query);

    return NextResponse.json(
      weeklySalesReadResponseSchema.parse({ ...result, requestId }),
    );
  } catch (error) {
    const unauthorized =
    ...
    reportUnexpectedApiError({
      ...
      requestId,
      route: "/api/stores/[storeId]/sales/weekly",
      method: "GET",
    });
      ...
      apiErrorSchema.parse({
        code: unauthorized
          ...
          : invalid
            ...
            : "WEEKLY_SALES_READ_FAILED",
        message: unauthorized
          ? "Magasin ou produit introuvable, ou accès refusé"
          : invalid
            ? "Plage de dates invalide"
            : "Les ventes hebdomadaires ne peuvent pas être calculées",
        requestId,
      }),

...
import { dailyImportPreviewResponseSchema } from "@/domain/imports/daily-schemas";
import { importUploadSchema } from "@/domain/imports/schemas";
...
import { createDailyMercalysPreview } from "@/server/services/daily-sales-import-service";
...
export async function POST(request: Request, routeContext: RouteContext) {
  ...
  try {
    ...
    const record = await createDailyMercalysPreview({
      context,
      fileName: input.file.name,
      bytes: new Uint8Array(await input.file.arrayBuffer()),
    });
    return NextResponse.json(
      dailyImportPreviewResponseSchema.parse({
        ...record.preview,
      ...
      apiErrorSchema.parse({
        code: unauthorized
          ...
          : codeMismatch
            ...
            : "DAILY_IMPORT_PREVIEW_FAILED",
        message: unauthorized
          ? "Magasin introuvable ou accès refusé"
          : codeMismatch
            ? error.message
            : "Le fichier journalier Mercalys n’a pas pu être prévisualisé",
        requestId,
      }),
